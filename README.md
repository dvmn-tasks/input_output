...
В Python для этих целей предусмотрено значение ```rating = None```. А строку ```"No rating yet"``` 
можно переместить туда где данные подготавливаются к выводу в **xlsx**.
...
выгрузки в xlsx поместить в ``` def fill_xlsx(workbook, courses): ```  
