...
кеша.
Как все это сделать когда `get_courses_list` сама завершает программу? От 
...
многократных вызовов `get_courses_list`.
...
диске.
Ага, значит вызывать `fetch_html` внутри `get_courses_list` не такая уж 
хорошая идея. Жить будет легче если передать в `get_courses_list` строку с 
...
выгрузки в xlsx поместить в `fill_xlsx(workbook, courses)`, а вывод в 
консоль собрать внутри main. Удастся избежать вычитывания 
