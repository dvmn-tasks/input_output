...
диске.
3. Ага, значит вызывать `fetch_html()` внутри `def get_courses_list` не такая уж 
хорошая идея. 
4. Жить будет легче если передать в `def get_courses_list` строку с 
HTML разметкой вместо `courses_url`. 
...
1. 1)от источника данных;
2. 2)от формата вывода в файл.
