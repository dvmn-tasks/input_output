...
другим порядком столбцов, как это сделать? Как заменить столбец ```python2_date``` на 
```pythondays_before_start``` ?
...
выгрузки в xlsx поместить в ```pythondef fill_xlsx(workbook, courses):```, а вывод в 
