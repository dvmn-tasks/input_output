...
кеша.
Как все это сделать когда ``` Python def get_courses_list ``` сама завершает программу ?! От
вызова ``` Python exit() ``` надо отказаться. Можно выбросить исключение и таким образом
сообщить о проблеме внешнему коду, пускай там разбираются.

Вызов ``` Python print ``` тоже стоит вынести из тела функции наружу. В рассмотренных
сценариях вывод в консоль зависит от общей логики загрузки данных и
многократных вызовов ``` Python def get_courses_list ```.
...
диске.
Ага, значит вызывать ``` Python fetch_html() ``` внутри ``` Python def get_courses_list ``` не такая уж
хорошая идея. Жить будет легче если передать в ``` Python def get_courses_list ``` строку с
HTML разметкой вместо ``` Python courses_url ```. Вуаля, мы решили проблемы еще до их
...
В Python для этих целей предусмотрено значение ``` Python rating = None ```. А строку "No
...
другим порядком столбцов, как это сделать? Как заменить столбец ``` Python 2_date ```на
``` Python days_before_start ``` ?
...
выгрузки в xlsx поместить в ``` Python def fill_xlsx(workbook, courses): ```, а вывод в
консоль собрать внутри ``` Python if __name__=='__main__': ```. Удастся избежать вычитывания
